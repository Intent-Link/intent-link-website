...
const quickstart: DocText = {
  ...
  "intro": "インストールから動く予測まで、数分で到達できます。intent-link はポインターの動きを監視し、カルマンフィルターでどこへ向かっているかを推定し、リンクが有力な遷移先になった時点で — クリックの前に — onIntent を発火します。",
  "installHeading": "1 · インストール",
  "peerDeps": "ピア依存関係: `react ≥ 18`、`react-dom ≥ 18`、`next ≥ 13`。",
  "wrapHeading": "2 · アプリを一度ラップする",
  "wrapBody": "ルート近くに `IntentProvider` をマウントします。予測エンジンを実行し、登録済みのすべての遷移先についてリアルタイムの確率を計算します。クライアントコンポーネントなので、App Router では `\"use client\"` 境界の中に置く必要があります。",
